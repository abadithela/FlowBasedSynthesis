...
def draw_maze(orig_maze, merge = False):
    ...
    for i in np.arange(0,size[0]+1):
        for k in np.arange(0,size[1]+1):
            ...
            if maze[(i,k)] != '*':
                if i == 0 and k == 0:
                    tile = patches.Rectangle((z_tiles[k],x_tiles[i]),TILESIZE,TILESIZE,linewidth=1,facecolor='#ffb000', alpha=0.5)
                elif i == 2 and k == 4:
                    tile = patches.Rectangle((z_tiles[k],x_tiles[i]),TILESIZE,TILESIZE,linewidth=1,facecolor='#648fff', alpha=0.5)
                elif i == 4 and k == 4:
                    tile = patches.Rectangle((z_tiles[k],x_tiles[i]),TILESIZE,TILESIZE,linewidth=1,facecolor='#dc267f', alpha=0.5)
                elif k == 1:
                    tile = patches.Rectangle((z_tiles[k],x_tiles[i]),TILESIZE,TILESIZE,linewidth=1,facecolor='lightskyblue', alpha=0.2)
                else:
...
def draw_timestamp(t, merge = False):
    if merge:
        ...
        ax.text(0.3,0.7,t, transform=plt.gcf().transFigure,fontsize='large',
             bbox={"boxstyle" : "circle", "color":"white", "ec":"black"})
def draw_tester(tester_pos):
    y_tile = tester_pos[0]
    x_tile = tester_pos[1]
    x = (x_tile + 0.5) * TILESIZE
    z = (y_tile + 0.5) * TILESIZE
    tester = patches.RegularPolygon((x,z), 5, radius=(TILESIZE-10)/2,facecolor='darkblue')
    # tester = patches.Circle((x,z), (TILESIZE-10)/2, linewidth=1,facecolor='black')
    ax.add_artist(tester)

def draw_system(sys_data):
    y_tile = sys_data[0][0]
    x_tile = sys_data[0][1]
    fuel_status = sys_data[-1]
    x = (x_tile + 0.5) * TILESIZE
    z = (y_tile + 0.5) * TILESIZE
    background = patches.Circle((x,z), (TILESIZE-10)/2, linewidth=1,facecolor='white')
    if fuel_status == 1:
        system = patches.Circle((x,z), (TILESIZE-10)/2, linewidth=1,facecolor='red')
    else:
        level = fuel_status*360
        system = patches.Wedge((x,z), (TILESIZE-10)/2, 0, level, facecolor='red')
    ax.add_artist(background)
    ax.add_artist(system)
...
def traces_to_animation(filename, output_dir):
    ...
    for t in t_array:
        ...
        draw_tester(traces[t].snapshot['test'])
        draw_system(traces[t].snapshot['sys'])
        # car_data = traces[t].snapshot['sys']
