# Synthesizing a controller for the tester
# Tester specifications: <>T /\ <>I1 /\ <>i2
...
class Quadruped_Tester:
    ...
    def add_target_dynamics(self):
        target_dyn = set()
        # target_dyn |= {'target = 0 -> X(target = 0 || target = 1)'}
        target_dyn |= {'target = 1 -> X(target = 1)'}
        self.T = self.maze.goal
        visited_T = {'('+ self.sys_zstr + '=' + str(self.T[0]) + ' & ' + self.sys_xstr + '=' + str(self.T[1]) +') <-> (target=1)'}
        target_dyn |= visited_T
        return target_dyn
    ...
    def add_target_progress(self):
        target_progress = {'target=1'}
    ...
    def find_controller(self,maze,tester):
        ...
        # dunp the controller
        controller_namestr = "tester_controller_target"+str(self.index)+".py"
