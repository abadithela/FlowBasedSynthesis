...
def highlight_history_cuts(cuts, GD, SD, virtual, virtual_sys):
    ...
    for q, reactive_cuts_at_q in reactive_cuts.items():
        ...
        virtual_sys.plot_with_highlighted_edges(sys_cuts_annot, f"imgs/virtual_sys_with_cuts_{q}")

    for q, reactive_cuts_at_q in reactive_cuts.items():
        virtual_cuts_annot = [((cut[0], q1), (cut[1], q2)) for cut in reactive_cuts_at_q for q1 in virtual.AP for q2 in virtual.AP]
        virtual.plot_with_highlighted_edges(virtual_cuts_annot, f"imgs/virtual_with_cuts_{q}")
    st()
def make_history_plots(cuts, GD, maze):
    ...
    num_panels = 0
    relevant_qs = []
    for q, reactive_q_cuts in reactive_cuts.items():
        if len(reactive_q_cuts)>0:
            relevant_qs.append(q)
    num_panels = len(relevant_qs)
    ...
    if num_panels > 1:
        ...
        for k,q in enumerate(relevant_qs):
