...
# max_rows = 4
# INIT = [((max_rows-1,max_rows-1),MAX_FUEL)]
# GOALS = [((max_rows-1,0),f) for f in range(0,MAX_FUEL)]

# LOW_FUEL = []
# for z in range(0,max_rows):
#     for x in range(0,max_rows):
#         for f in range(1,min(((max_rows-1-z)+x-1),MAX_FUEL)):
#             LOW_FUEL.append(((z,x),f))

# LOW_FUEL = list(set(LOW_FUEL))
# EMPTY = [((x,y),0) for x in range(0,max_rows) for y in range(0,max_rows)]
...
# STATIC_AREA = [((z,x),f) for z in range(0,max_rows) for x in range(0,max_rows) for f in range(0,MAX_FUEL+1) if not x == 2]
