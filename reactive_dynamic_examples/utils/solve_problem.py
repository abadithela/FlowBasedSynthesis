...
def solve_problem_augmented(virtual, system, b_pi, virtual_sys, static_area, excluded_sols = [], print_solution=True, plot_results=True, alpha=0.8):
    GD, SD = setup_nodes_and_edges(virtual, virtual_sys, b_pi)

    ti = time.time()
    exit_status, ftest, d, flow = solve_max_gurobi_augmented(GD, SD, static_area = static_area, excluded_sols=excluded_sols, alpha = 0.8)
    tf = time.time()
    del_t = tf-ti
