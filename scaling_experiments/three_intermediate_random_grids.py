'''
Solving empty grids of different sizes with three intermediates using the MILP for static obstacles.
'''
...
def plot_runtimes(runtimes):
    ...
    ax.set(xlabel='Grid Size N', ylabel='Runtime (s)',
           title='Runtime vs. NxN Grid for 3 Intermediates')
    ax.grid()
    ax.legend(loc="upper left")
    ax.set_facecolor('whitesmoke')
    plt.grid(True,linestyle='--')
    fig.savefig("imgs/runtimes_3_int.pdf")
    plt.show()
...
if __name__ == '__main__':
    ...
    for gridsize in mazefiles.keys():
        ...
        for run in range(number_of_runs):
            ...
            int_3 = all_states[idx[3]]
            goals = [all_states[idx[4]]]
            obs = [all_states[idx[5+int(n)]] for n in np.arange(0,obsnum)]
            ...
            delta_t_g = tf-ti
            print('{} s to make the graph'.format(delta_t_g))
            # st()

            print("{0}: S = {1}, I1 = {2}, I2={3}, I3={4} T = {5}".format(gridsize, init, int_1, int_2, int_3, goals))

            exit_status, del_t, annot_cuts, flow, bypass = solve_instance(virtual, system, b_pi, virtual_sys)#, focus, cuts, presolve)
            # print("{0}: S = {1}, I1 = {2}, I2={3}, I3={4} T = {5}".format(gridsize, init, int_1, int_2, int_3, goals))
            #
            if exit_status == 'opt':
