...
def sync_eventually(state_str="q"):
    """
    Defining automaton for the simple eventually-eventually asynchronous product
    """
    nstates = 4
    ...
    interm = AP[1]
    tau = {
            ("q0", spot.formula.And([sink, interm])): "q3",
            ("q0", spot.formula.And([sink,get_neg(interm)])): "q2",
            ("q0", spot.formula.And([get_neg(sink),get_neg(interm)])): "q0",
            ("q0",spot.formula.And([get_neg(sink),interm])): "q1",
    ...
    Acc = {"sys": ("q2","q3"), "test": ("q1","q3")} # accepting sets of states
    return Q, qinit, AP, tau, Acc, Qdict
